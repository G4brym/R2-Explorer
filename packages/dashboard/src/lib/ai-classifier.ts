...
export async function classifyDocumentWithAI(
	...
	console.log(
		"🤖 AI document classification for:",
		filename,
		"Type:",
		file.type,
	);
...
function classifyByFilename(filename: string): DocumentClassification {
	...
	if (
		["invoice", "inv", "bill", "statement", "payment"].some((k) =>
			lower.includes(k),
		)
	) {
		return {
			...
			confidence: 0.6,
			summary:
				"Invoice document identified by filename. Manual review recommended.",
			priority: "NORMAL",
		};
	}
	if (
		["contract", "agreement", "msa", "sow", "terms"].some((k) =>
			lower.includes(k),
		)
	) {
		return {
			...
			confidence: 0.6,
			summary:
				"Contract document identified by filename. Manual review recommended.",
			priority: "NORMAL",
		};
	}
	if (
		["workflow", "process", "diagram", "flow", "procedure"].some((k) =>
			lower.includes(k),
		)
	) {
		return {
			...
			confidence: 0.6,
			summary:
				"Workflow document identified by filename. Manual review recommended.",
			priority: "NORMAL",
		};
	}
