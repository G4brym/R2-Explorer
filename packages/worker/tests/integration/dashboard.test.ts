...
describe("Dashboard Endpoints", () => {
    describe("dashboardIndex (GET /)", () => {
        it("should return 500 if ASSETS binding is undefined", async () => {
            ...
            const response = await app.fetch(request, currentEnv, createExecutionContext());
        ...
        it("should return 500 if ASSETS.fetch is not a function (simulating invalid ASSETS binding)", async () => {
            ...
            expect(text).toContain("ASSETS binding is not pointing to a valid dashboard");
        });

        it("should call ASSETS.fetch and return its response if ASSETS is valid", async () => {
            const mockAssetsResponse = new Response("Dashboard Content", { status: 200 });
            const mockAssets = { fetch: vi.fn().mockResolvedValue(mockAssetsResponse) };
            const app = createTestApp();
            const request = createTestRequest('/');
            ...
            expect(response.status).toBe(500);
            expect(await response.text()).toContain("ASSETS binding is not pointing to a valid dashboard");
        });
    });
    ...
    describe("dashboardRedirect (GET *)", () => {
        ...
        it("path w/o dot, ASSETS defined: should call ASSETS.fetch", async () => {
            const mockAssetsResponse = new Response("SPA Route Content", { status: 200 });
            const mockAssets = { fetch: vi.fn().mockResolvedValue(mockAssetsResponse) };
            const app = createTestApp();
